...
const REPLY_TO = process.env.RESEND_REPLY_TO || 'hackathon@foundersfest.org'
 ...
 *   subject     – email subject (sent as-is)
...
export async function sendEmail({
  ...
  try {
    ...
    if (provider === 'smtp') {
      ...
      const fromAddress = process.env.SMTP_FROM_EMAIL || FROM
      ...
      // Pace bulk for...of loops to stay within SMTP server rate limits
      const delayMs = parseInt(process.env.SMTP_DELAY_MS || '200')
    ...
    console.log(`[email] sent (${provider}): ${emailType} → ${to} (${messageId})`)
